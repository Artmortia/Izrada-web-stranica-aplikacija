...
<body>
	...
	<div id="main-wrapper">
		...
		<br>
			<center><h2>Narudzbe Korisnika</h2></center>
			<?php
		...
		<br>
			...
			<section id="sekcija">
				<div id = "sekcija_tekst"> 
					...
					<center><h3>Narudzbe Korisnika</h3></center>
					...
					<?php
						if(isset($_SESSION["id"])){
							...
							if(isset($_GET["id_narudzbe_P"])){
								...
								$datum = date("Y-m-d");
								$datum_isporuke = date('Y-m-d', strtotime($datum.' + 15 days'));
								$vrijeme_isporuke = date("H:i:s");
								$upit = "UPDATE narudzba SET
										status = 'P',
										datum_isporuke = '$datum_isporuke',
										vrijeme_isporuke = '$vrijeme_isporuke'										
										WHERE narudzba_id = '$id_narudzba'";
								izvrsiUpit($baza_pod,$upit);
							}
							...
							while ($red=mysqli_fetch_array($rezultat)){
									...
									$sql2="SELECT * FROM narudzba WHERE status = 'K'";
									...
									while($red2=mysqli_fetch_array($rezultat2)){
										...
										if($red['narudzba_id']==$red2['narudzba_id']){
											...
											echo "<td><b><a href='narudzbe_korisnika.php?id_narudzbe_P=".$red2['narudzba_id']."'>Prihvati</a></b></td>";
											echo "<td><b><a href='narudzbe_korisnika.php?id_narudzbe=".$red2['narudzba_id']."'>Odbij</a></b></td>";
											echo "</tr>";
